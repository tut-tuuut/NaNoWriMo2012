...
\newcommand{\done}{\item[$\blacksquaref$]}
...
\todo BP if they actually do it
\todo DBP if they convince someone to do it with them
\todo TBP if they figure out the crucial element of the plot while bungee jumping
\todo Starbucks points if YOU go bungee jumping for the sake of research.
\ei

\bi
\todo Have characters randomly quote the hogwarts sorting hat songs.
...
\bi
\todo Let your quietest character (main or not) have the biggest secret.
\todo BP if it's a dangerous secret.
\todo TBP if they've been hiding it since WAY before the novel started.
\ei
...
\todo BP if their real name is incredibly plain and kind of forgettable.
...
\todo SBP if the name is integral to the plot somehow.
\todo A cookie if the name is revealed in the final paragraph.
\todo A chocolate fish if no one knows where the nickname came from ANYWAY.
\ei


\bi
\todo Have your characters play a game of One Thousand Blank White Cards. If you don't know how it's played, the explanation is here: \texttt{http://www.geocities.com/nconner23/bwcards.html}
\ei


\bi
\todo Have your main characters get stuck in a traffic jam on their way to Meet the Evil Nemisis at You-Know-Where for the Final Showdown so they can Save the World.
\todo + if the villain rides past on a bicycle
\todo ++ if your story is set in a time period when neither cars nor bicycles are invented
\todo +++ if the villain rides through the traffic at the exact moment that it finally moves -- and gets hit
\ei
...
\bi
\todo One of your characters always says “Thank you for calling Dominoes, how may I help you” every time they pick up the phone (instead of Hello).
\ei
...
\item Ex: Like talk they this would. Annoying is not this?
